...
class TestImageUploadEndpoint:
    def test_successful_image_upload(self, test_client, sample_image_bytes):
        with patch(
            "src.image_processing_service.app.services.ProcessingOrchestrator.start_image_processing"
        ) as mock_start:
            image_id = str(uuid4())
    ...
    def test_upload_processing_error(self, test_client, sample_image_bytes):
        with patch(
            "src.image_processing_service.app.services.ProcessingOrchestrator.start_image_processing"
        ) as mock_start:
            mock_start.side_effect = ValueError("Invalid image format")
            response = test_client.post(
                "/api/modify",
                files={"file": ("test.jpg", sample_image_bytes, "image/jpeg")},
            )
...
class TestProcessingStatusEndpoint:
    def test_get_processing_status_success(self, test_client):
        processing_id = uuid4()

        with patch(
            "src.image_processing_service.app.services.ProcessingOrchestrator.get_processing_status"
        ) as mock_status:
    ...
    def test_get_processing_status_not_found(self, test_client):
...
class TestModificationDetailsEndpoint:
    def test_get_modification_details_success(self, test_client):
        modification_id = uuid4()

        with patch(
            "src.image_processing_service.app.services.ProcessingOrchestrator.get_modification_details"
        ) as mock_details:
            ...
            mock_details.return_value = mock_result
    ...
    def test_get_modification_details_not_found(self, test_client):
        modification_id = uuid4()
...
class TestOriginalImageEndpoint:
    def test_serve_original_image_success(self, test_client, sample_image_bytes):
        image_id = uuid4()
        ...
        try:
            with patch("src.image_processing_service.app.models.Image.get") as mock_get:
                with patch(
                    "src.image_processing_service.app.services.file_storage.FileStorageService.file_exists"
                ) as mock_exists:
                    mock_image = AsyncMock()
    ...
    def test_serve_original_image_file_missing(self, test_client):
        image_id = uuid4()
        with patch("src.image_processing_service.app.models.Image.get") as mock_get:
            mock_image = AsyncMock()
...
class TestVariantListEndpoint:
    def test_list_image_variants_success(self, test_client):
        image_id = uuid4()

        with patch(
            "src.image_processing_service.app.services.ProcessingOrchestrator.get_image_variants"
        ) as mock_variants:
            mock_modifications = []
            for i in range(3):
                mock_mod = AsyncMock()
            ...
            mock_variants.return_value = mock_modifications

            response = test_client.get(f"/api/images/{image_id}/variants")

            assert response.status_code == 500

    def test_list_image_variants_not_found(self, test_client):
        image_id = uuid4()
        with patch(
            "src.image_processing_service.app.services.ProcessingOrchestrator.get_image_variants"
        ) as mock_variants:
    ...
    def test_list_image_variants_empty(self, test_client):
        image_id = uuid4()
        with patch(
            "src.image_processing_service.app.services.ProcessingOrchestrator.get_image_variants"
        ) as mock_variants:
...
class TestEndToEndWorkflow:
    ...
    def test_complete_image_processing_workflow(self, test_client, sample_image_bytes):
        # Step 1: Upload image
        with patch(
            "src.image_processing_service.app.services.ProcessingOrchestrator.start_image_processing"
        ) as mock_start:
        ...
        with patch(
            ...
            mock_status.return_value = self._create_mock_status_result(processing_id)
        ...
        # Step 3: Get modification details
        with patch(
            "src.image_processing_service.app.services.ProcessingOrchestrator.get_modification_details"
        ) as mock_details:
            mock_details.return_value = self._create_mock_modification_details(
                processing_id, "workflow_test.jpg"
            )
        ...
        with patch(
            ...
            mock_variants.return_value = self._create_mock_variants(100)
